...
function rejectWithToast(error: unknown, silent = false) {
  ...
  async (error) => {
    ...
    if (
      ...
      if (!refreshToken) {
        ...
        return rejectWithToast(error, true);
      }
      ...
      try {
        const { data } = await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:4000"}/v1/auth/refresh`,
          { refreshToken },
        );
        ...
        clearTokens();
        window.dispatchEvent(new CustomEvent("auth:unauthorized"));
        return rejectWithToast(refreshError, true);
      } finally {
        isRefreshing = false;
      }
    }
